...
earthly:
    FROM +devcontainer-base
    ARG TARGETARCH
    RUN curl -fsSL https://github.com/earthly/earthly/releases/download/v0.6.19/earthly-linux-${TARGETARCH} -o /usr/local/bin/earthly \
...
dts2hx:
    ...
    RUN npx dts2hx @types/node cross-fetch @types/chrome abort-controller @types/gtag.js fastify sitemap nroonga --noLibWrap --useSystemHaxe --output lib/dts2hx
